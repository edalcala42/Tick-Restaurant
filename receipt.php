...
$order = $conn->query("SELECT * FROM factura where id = {$_GET['id']}");
foreach($order->fetch_array() as $k => $v){
	$$k= $v;
}
$items = $conn->query("SELECT o.*,p.nombre FROM pedido o inner join producto p on p.id = o.idProducto where o.idFactura = $id ");
?>
	...
	<table width="100%">
		...
		<tbody>
			...
			<tr>
				...
				<td><p><?php echo $row['nombre'] ?></p><?php if($row['cantidada'] > 0): ?><small>(<?php echo number_format($row['precio'],2) ?>)</small> <?php endif; ?></td>
				<td class="text-right"><?php echo number_format($row['pago'],2) ?></td>
			</tr>
			<?php endwhile; ?>
		</tbody>
	</table>
